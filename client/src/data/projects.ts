...
export const projects: Project[] = [
  ...
  },
  {
    ...
    projectUrls: [
      { label: "项目站点", href: "https://disclose-luke-replace-bite.trycloudflare.com" },
      { label: "GitHub 源码", href: "https://github.com/shuhari04/durian" },
    ],
    description:
      "榴莲相机是一个本地运行的 AI 选榴莲助手。用户上传榴莲照片后,系统会在 Mac 本机完成图像分析,不调用云端 API,用于判断榴莲的外观成熟度、整体品相和购买风险,帮助用户更快做出『推荐购买、可以购买、谨慎购买或不建议购买』的决策。当前版本是原型优先实现,复用了开源榴莲成熟度识别模型,并结合本地 OpenCV 图像分析规则生成品相评分。功能包括:照片上传分析(支持 JPG/PNG/HEIC,单张最大 20MB)、本地 AI 推理(FastAPI + PyTorch)、成熟度识别(未成熟/接近成熟/成熟/过熟 + 模型置信度)、品相评分(0-100 分)、直接输出购买建议、风险提示、标注结果图、最近记录与模型状态显示。",
    vibeCoding: "",
    feeling:
